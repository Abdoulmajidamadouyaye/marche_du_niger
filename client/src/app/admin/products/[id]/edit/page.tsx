...
const AdminEditProductPage = () => {
  ...
  useEffect(() => {
    ...
    const initialImages = Object.values(product.images ?? {}).filter(Boolean);
    const safeFirstImage = initialImages[0] ?? "";
    ...
    setForm({
      ...
      imageUrl: safeFirstImage,
      country: product.country as Country | undefined,
      regions: (product.regions ?? []) as string[],
      shippingDelayDays:
        product.shippingDelayDays !== undefined && product.shippingDelayDays !== null
          ? String(product.shippingDelayDays)
          : "",
    });
    setGalleryImageUrls(initialImages);
    setLocalImageNames([]);

    if (product.vehicleSpecs) {
      setVehicleSpecs({
        brand: product.vehicleSpecs.brand,
        model: product.vehicleSpecs.model,
        year: String(product.vehicleSpecs.year),
        engineType: product.vehicleSpecs.engineType,
        horsepower: String(product.vehicleSpecs.horsepower),
        transmission: product.vehicleSpecs.transmission,
        fuelConsumption: String(product.vehicleSpecs.fuelConsumption),
        autonomy: String(product.vehicleSpecs.autonomy),
        tankCapacity: String(product.vehicleSpecs.tankCapacity),
        mileage: String(product.vehicleSpecs.mileage),
        doors: String(product.vehicleSpecs.doors),
        seats: String(product.vehicleSpecs.seats),
        color: product.vehicleSpecs.color,
        airbags: product.vehicleSpecs.airbags,
        abs: product.vehicleSpecs.abs,
        reverseCamera: product.vehicleSpecs.reverseCamera,
        airConditioning: product.vehicleSpecs.airConditioning,
        gpsNavigation: product.vehicleSpecs.gpsNavigation,
        touchScreen: product.vehicleSpecs.touchScreen,
        condition: product.vehicleSpecs.condition,
        papersAvailable: product.vehicleSpecs.papersAvailable,
      });
    } else {
      setVehicleSpecs(defaultVehicleSpecsFormData);
    }

    if (product.motoSpecs) {
      setMotoSpecs({
        brand: product.motoSpecs.brand,
        model: product.motoSpecs.model,
        year: String(product.motoSpecs.year),
        motoType: product.motoSpecs.motoType,
        engineType: product.motoSpecs.engineType,
        maxSpeed: String(product.motoSpecs.maxSpeed),
        weight: String(product.motoSpecs.weight),
        color: product.motoSpecs.color,
        tireType: product.motoSpecs.tireType,
        mileage: String(product.motoSpecs.mileage),
        usage: product.motoSpecs.usage,
        fuelConsumption: String(product.motoSpecs.fuelConsumption),
        tankCapacity: String(product.motoSpecs.tankCapacity),
        brakes: product.motoSpecs.brakes,
        antiTheft: product.motoSpecs.antiTheft,
        ledLighting: product.motoSpecs.ledLighting,
        electricStart: product.motoSpecs.electricStart,
        digitalDashboard: product.motoSpecs.digitalDashboard,
        condition: product.motoSpecs.condition,
        papersAvailable: product.motoSpecs.papersAvailable,
      });
    } else {
      setMotoSpecs(defaultMotoSpecsFormData);
    }
  }, [product]);
  ...
  if (!product) {
    return (
      <div className="p-8">
        ...
        <Link href="/admin/products" className="text-sm text-emerald-600 hover:underline mt-2 block">
          Retour aux produits
        </Link>
      </div>
    );
  ...
  const validate = (): boolean => {
    ...
    if (!form.shortDescription.trim()) newErrors.shortDescription = "Requis.";
    if (!form.description.trim()) newErrors.description = "Requise.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    setApiError("");
    notify("Mise a jour du produit...", "info", 1800);
    try {
      await updateProduct(product.id, updates);
      notify("Produit mis a jour avec succes.", "success", 2800);
      router.push("/admin/products");
    } catch {
      setApiError("Erreur lors de la mise à jour. Vérifiez que le serveur est démarré.");
      notify("Erreur lors de la mise a jour du produit.", "error", 3600);
      setSaving(false);
    }
  };
  ...
  return (
    <div className="mx-auto w-full max-w-4xl pb-12">
      ...
      <form onSubmit={handleSubmit} className="space-y-5 rounded-xl border border-gray-200 bg-white p-4 sm:p-6">
        ...
        <Field label="Images du produit (URLs ou fichiers)" error={errors.imageUrl}>
          <div className="space-y-2">
            <textarea
              ...
              onChange={(e) => {
                const list = e.target.value
                  ...
                  .filter(Boolean);
                setGalleryImageUrls(list);
                setForm((prev) => ({ ...prev, imageUrl: list[0] ?? "" }));
              }}
            ...
            {localImageNames.length > 0 && (
              <p className="text-xs text-gray-600">{localImageNames.length} image(s) locale(s) ajoutee(s)</p>
            )}
        ...
        <Field label="Tailles (séparées par virgules, optionnel)">
          <input
            type="text"
            value={form.sizes}
        ...
        <div className="flex flex-col gap-3 pt-2 sm:flex-row">
          <button
            type="submit"
            disabled={saving}
            className="rounded-md bg-emerald-600 px-5 py-2 text-sm font-semibold text-white hover:bg-emerald-700 transition disabled:opacity-60"
          >
